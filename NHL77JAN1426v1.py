                    ...
                    try:
                        cursor_save.execute('''
                            INSERT INTO predictions (
                                ...
                                elo_home_prob, xgboost_home_prob, logistic_home_prob, win_probability, locked
                            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)
                        ''', (
...
def calculate_nhl_weekly_performance():
    ...
    try:
        ...
        # Get all completed NHL games with predictions
        games = conn.execute('''
...
def calculate_nba_weekly_performance():
    ...
    try:
        ...
        # Get all completed NBA games with predictions
        games = conn.execute('''
            SELECT g.game_date, g.home_team_id, g.away_team_id,
                   g.home_score, g.away_score,
            ...
            WHERE g.sport = 'NBA' AND date(g.game_date) >= '2025-10-21'
            ORDER BY g.game_date
        ''').fetchall()
        ...
        # Group games by week (calculate week number from season start)
        season_start = datetime(2025, 10, 21)  # NBA season starts Oct 21, 2025
...
DAILY_RESULTS_TEMPLATE = BASE_TEMPLATE.replace(
    ...
    {% if daily_results %}
        ...
        {% for date in sorted_dates|reverse %}
...
def sport_results(sport):
    ...
    if sport == 'NHL':
        ...
        try:
            ...
            sorted_dates = [d for d in sorted_dates if d <= yesterday]
    ...
    if sport in ['NCAAB', 'NCAAF', 'MLB', 'WNBA']:
        ...
        for game in completed_games:
            ...
            # Calculate win based on scores
            home_won = game['home_score'] > game['away_score']
            
            # Get prediction probability (use ensemble if available)
            home_prob = game['win_probability'] or game['elo_home_prob'] or 0.5
            ...
            game_info = {
                ...
                'home_win': home_won,
                'correct': correct,
                'home_elo': round(float(game['elo_home_prob'] or 0.5) * 100, 1),
                'home_xgb': round(float(game['xgboost_home_prob'] or game['elo_home_prob'] or 0.5) * 100, 1),
                'home_cat': round(float(game['logistic_home_prob'] or game['elo_home_prob'] or 0.5) * 100, 1),
                'home_meta': round(float(game['win_probability'] or game['elo_home_prob'] or 0.5) * 100, 1),
            }
            
            daily_results[game_info['date']]['games'].append(game_info)
        
        sorted_dates = sorted(daily_results.keys(), reverse=True)[:30]  # Last 30 days with games
...
SPREAD_TOTAL_RESULTS_TEMPLATE = BASE_TEMPLATE.replace(
    ...
    .tab { padding: 12px 30px; border-radius: 8px; text-decoration: none; font-weight: 600; transition: all 0.3s; background: rgba(255, 255, 255, 0.1); color: white; }
    .tab.active { background: linear-gradient(135deg, #10b981, #059669); }
    .filter-toggles { display: flex; gap: 10px; justify-content: center; margin-bottom: 20px; }
    ...
    .date-section { background: rgba(255, 255, 255, 0.05); border-radius: 15px; padding: 25px; margin-bottom: 30px; }
    .date-header { color: #fbbf24; font-size: 1.5em; margin-bottom: 15px; padding-bottom: 10px; border-bottom: 2px solid rgba(255, 255, 255, 0.2); }
    .games-table { width: 100%; border-collapse: separate; border-spacing: 0; font-size: 0.9em; }
    .games-table th { background: rgba(255, 255, 255, 0.15); padding: 12px 10px; font-weight: bold; color: #fbbf24; border-bottom: 2px solid rgba(255, 255, 255, 0.3); position: sticky; top: 0; }
    ...
    .games-table td { padding: 12px 10px; border-bottom: 1px solid rgba(255, 255, 255, 0.1); }
    ...
    .overall-stats { background: linear-gradient(135deg, #10b981, #059669); border-radius: 15px; padding: 30px; margin-bottom: 30px; }
    .overall-stats h2 { margin: 0 0 25px 0; font-size: 1.8em; text-align: center; }
    .stats-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px; }
    .stat-card { background: rgba(255, 255, 255, 0.15); border-radius: 12px; padding: 20px; text-align: center; transition: transform 0.2s; }
    .stat-card.primary { background: rgba(255, 255, 255, 0.25); border: 2px solid rgba(255, 255, 255, 0.4); transform: scale(1.02); }
    .stat-card:hover { transform: translateY(-2px); }
    .stat-card-label { font-size: 0.85em; opacity: 0.9; margin-bottom: 8px; text-transform: uppercase; letter-spacing: 0.5px; }
    .stat-card-value { font-size: 2.2em; font-weight: bold; margin-bottom: 5px; }
    .stat-card.primary .stat-card-value { font-size: 2.5em; }
    .stat-card-detail { font-size: 0.9em; opacity: 0.85; }
    .daily-stats { display: grid; grid-template-columns: repeat(3, 1fr); gap: 15px; margin-bottom: 20px; }
    .daily-stat-card { background: rgba(255, 255, 255, 0.1); border-radius: 10px; padding: 15px; text-align: center; }
    .stat-label { font-size: 0.9em; opacity: 0.8; margin-bottom: 5px; }
    .stat-value { font-size: 1.8em; font-weight: bold; color: #fbbf24; }
    .stat-record { font-size: 0.9em; opacity: 0.9; }
    """
).replace('{% block content %}{% endblock %}', """
    <div style="margin-bottom: 20px;">
        <a href="/" style="display: inline-block; padding: 10px 20px; background: rgba(255,255,255,0.1); border-radius: 8px; text-decoration: none; color: white; font-weight: 600;">← Back to Home</a>
    ...
    {% if daily_results %}
        <div class="overall-stats">
            ...
            <div class="stats-grid">
                <div class="stat-card">
                    <div class="stat-card-label">Total Games Tracked</div>
                    <div class="stat-card-value">{{ total_games }}</div>
                </div>
                <div class="stat-card primary">
                    <div class="stat-card-label">Model Spread Accuracy</div>
                    <div class="stat-card-value">{{ "%.1f"|format(spread_correct_pct) }}%</div>
                    <div class="stat-card-detail">{{ total_spread_correct }}-{{ total_games - total_spread_correct }} (Winner Prediction)</div>
                </div>
                <div class="stat-card">
                    <div class="stat-card-label">Model Over Record</div>
                    <div class="stat-card-value">{{ total_over }}-{{ total_under }}</div>
                    <div class="stat-card-detail">{{ "%.1f"|format(over_pct) }}% OVER</div>
                </div>
                <div class="stat-card">
                    <div class="stat-card-label">Avg Spread Error</div>
                    <div class="stat-card-value">{{ "%.1f"|format(avg_spread_margin) }}</div>
                    <div class="stat-card-detail">pts/goal margin</div>
                </div>
            </div>
        </div>
        
        <div class="filter-toggles">
            <button class="filter-btn active" onclick="filterResults('all')">All Games</button>
            <button class="filter-btn" onclick="filterResults('correct')">✅ Correct Only</button>
            <button class="filter-btn" onclick="filterResults('incorrect')">❌ Incorrect Only</button>
        </div>
        ...
        <div id="date-{{ date }}" class="date-section">
            ...
            <div class="daily-stats">
                <div class="daily-stat-card">
                    <div class="stat-label">Model Spread Accuracy</div>
                    <div class="stat-value">{{ (date_data.spread_correct / date_data.total_games * 100)|round(1) if date_data.total_games > 0 else 0 }}%</div>
                    <div class="stat-record">{{ date_data.spread_correct }}-{{ date_data.total_games - date_data.spread_correct }}</div>
                </div>
                <div class="daily-stat-card">
                    <div class="stat-label">Over/Under Split</div>
                    <div class="stat-value">{{ date_data.over_count }}-{{ date_data.under_count }}</div>
                    <div class="stat-record">{{ (date_data.over_count / date_data.total_games * 100)|round(1) if date_data.total_games > 0 else 0 }}% OVER</div>
                </div>
                <div class="daily-stat-card">
            ...
            <table class="games-table">
                <thead>
                    <tr>
                        ...
                        <th class="align-center">Final Score</th>
                        <th class="align-right">Model Total<br><span style="font-size: 0.85em; opacity: 0.8;">(Predicted / Actual)</span></th>
                        <th class="align-center">O/U Result</th>
                        <th class="align-right">Model Spread<br><span style="font-size: 0.85em; opacity: 0.8;">(Predicted / Actual Margin)</span></th>
                        <th class="align-center">Model Correct</th>
                    </tr>
                </thead>
                <tbody>
                    {% for game in date_data.games %}
                    <tr data-correct="{{ 'true' if game.spread_correct else 'false' }}">
                        <td class="align-left">{{ date }}</td>
                        <td class="align-left"><strong>{{ game.matchup }}</strong></td>
                        <td class="align-center" style="font-weight: 700; font-size: 1.05em;">{{ game.score }}</td>
                        <td class="align-right">{{ "%.1f"|format(game.pred_total) }} / {{ game.actual_total }}</td>
                        <td class="align-center {% if game.over_result %}over{% else %}under{% endif %}" style="font-size: 1.05em;">{% if game.over_result %}⬆️ OVER{% else %}⬇️ UNDER{% endif %}</td>
                        <td class="align-right">{{ "%+.1f"|format(game.pred_spread) }} / {{ "%+.1f"|format(game.actual_spread) }}</td>
                        <td class="align-center {% if game.spread_correct %}result-correct{% else %}result-incorrect{% endif %}" style="font-size: 1.3em;">{% if game.spread_correct %}✅{% else %}❌{% endif %}</td>
                    </tr>
                    {% endfor %}
                </tbody>
            </table>
...
def sport_spread_total_results(sport):
    ...
    # Group games by date and calculate results
    ...
    for game in completed_games:
        try:
            ...
            # Over/Under result
            over_result = actual_total > pred_total
            if over_result:
                total_over += 1
            ...
            if spread_correct:
                ...
                daily_results[game['game_date']]['spread_correct'] += 1
    ...
    # Filter to yesterday and before, then sort newest first
    yesterday = (datetime.now() - timedelta(days=1)).strftime('%Y-%m-%d')
    ...
    return render_template_string(
        ...
        over_pct=over_pct,
        under_pct=under_pct,
        total_spread_correct=total_spread_correct,
    ...
    try:
        ...
        for pred in predictions:
            picks.append({
                ...
                'elo': pred.get('elo_prob'),
                'xgb': pred.get('xgb_prob'),
                'cat': pred.get('cat_prob')
            })
