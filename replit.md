...
The NHL prediction system for the 2025-26 season utilizes advanced ensemble models (Elo, XGBoost, CatBoost, Meta Ensemble) with comprehensive feature engineering (rolling windows, home/away splits, rest, strength of schedule, head-to-head history, goalie performance). Training uses 1,176 games from the 2024-25 season (loaded directly from nhlschedules.py file, NOT from database) with time-based split, recency weighting, and K-factor optimization. Model accuracies: Elo 54.7%, XGBoost 56.4%, CatBoost 58.5%, Meta ~57%. The system displays 1,132 games for the 2025-26 season (season=2025 in database, matching all other sports). **CRITICAL**: The season=2025 NHL schedule in the database must NEVER be modified - it is the current season prediction schedule shown to users.
