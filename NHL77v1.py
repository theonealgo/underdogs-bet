    ...
    for game_date, game in all_games_with_dates:
        ...
        if game_date >= season_start and game_date <= one_month_ahead:
            ...
            if game.get('stored_elo_prob') is not None:
                ...
                # Adaptive weighting for NFL: 70% Elo (proven best), 15% XGBoost, 15% CatBoost
                if sport == 'NFL':
                    ensemble_prob = (elo_prob * 0.70) + (xgb_prob * 0.15) + (cat_prob * 0.15)
            else:
                ...
                if sport == 'NHL':
                    try:
                        # Use advanced NHL feature engineering
                        nhl_engineer = NHLFeatureEngineer()
                        game_date_str = game.get('game_date', datetime.now().strftime('%Y-%m-%d'))
                        features = nhl_engineer.engineer_features(
                            game['home_team_id'], 
                            game['away_team_id'], 
                            game_date_str
                        )
                        ml_boost = nhl_engineer.calculate_probability_boost(features)
                    except Exception as e:
                        logger.debug(f"NHL feature engineering error: {e}")
                        ml_boost = 0.0
                
                elif sport == 'NBA':
                    try:
                        # Use advanced NBA feature engineering
                        nba_engineer = NBAFeatureEngineer()
                        game_date_str = game.get('game_date', datetime.now().strftime('%Y-%m-%d'))
                        features = nba_engineer.engineer_features(
                            game['home_team_id'], 
                            game['away_team_id'], 
                            game_date_str
                        )
                        ml_boost = nba_engineer.calculate_probability_boost(features)
                    except Exception as e:
                        logger.debug(f"NBA feature engineering error: {e}")
                        ml_boost = 0.0
                ...
                # Enhanced model predictions (now with ML boost)
                xgb_prob = min(0.95, max(0.05, elo_prob + goalie_boost + market_boost * 0.5 + split_boost + ml_boost * 0.3))
                cat_prob = min(0.95, max(0.05, elo_prob + goalie_boost * 0.7 + market_boost * 0.3 + split_boost * 0.5 + ml_boost * 0.4))
                ...
                # Apply ML boost directly to ensemble for NHL and NBA
                if sport in ['NHL', 'NBA']:
                    ensemble_prob = min(0.95, max(0.05, ensemble_prob + ml_boost))
...
def calculate_nfl_weekly_performance():
    ...
    try:
        ...
        for _, api_game in completed_games.iterrows():
            ...
            # Adaptive weighting for NFL: 70% Elo, 15% XGBoost, 15% CatBoost
            if elo_prob and xgb_prob and cat_prob:
                ens_prob = (elo_prob * 0.70) + (xgb_prob * 0.15) + (cat_prob * 0.15)
            else:
                ens_prob = elo_prob
...
def calculate_nba_weekly_performance():
    ...
    try:
        ...
        # Get all completed NBA games with predictions (show through yesterday)
        today_str = datetime.now().strftime('%Y-%m-%d')
        games = conn.execute('''
            SELECT g.game_date, g.home_team_id, g.away_team_id,
                   g.home_score, g.away_score,
            ...
            WHERE g.sport = 'NBA' AND date(g.game_date) >= '2024-10-21'
              AND date(g.game_date) <= date(?, '-1 day')
            ORDER BY g.game_date
        ''', (today_str,)).fetchall()
...
def calculate_model_performance(sport):
    ...
    # Calculate cutoff for results (show through yesterday, one day before today)
    today_str = datetime.now().strftime('%Y-%m-%d')
...
DAILY_RESULTS_TEMPLATE = BASE_TEMPLATE.replace(
    ...
    {% if daily_results %}
        ...
        <div style="background: linear-gradient(135deg, #10b981, #059669); border-radius: 15px; padding: 25px; margin-bottom: 30px; text-align: center;">
            ...
            {% set elo_total_correct = all_games|selectattr('elo_correct')|list|length %}
            {% set xgb_total_correct = all_games|selectattr('xgb_correct')|list|length %}
            {% set cat_total_correct = all_games|selectattr('cat_correct')|list|length %}
            {% set elo_total_wrong = total_games - elo_total_correct %}
            {% set xgb_total_wrong = total_games - xgb_total_correct %}
            {% set cat_total_wrong = total_games - cat_total_correct %}
            
            <div style="margin-top: 20px; padding-top: 20px; border-top: 2px solid rgba(255,255,255,0.3);">
                <div style="font-size: 1.1em; margin-bottom: 15px; font-weight: 600;">📊 Model Breakdown</div>
                <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px; font-size: 0.95em;">
                    <div style="background: rgba(255,255,255,0.1); padding: 12px; border-radius: 8px;">
                        <div style="opacity: 0.9; margin-bottom: 5px;">Elo</div>
                        <div style="font-size: 1.4em; font-weight: bold;">{{ elo_total_correct }}-{{ elo_total_wrong }}</div>
                        <div style="opacity: 0.8; font-size: 0.9em; margin-top: 3px;">{{ ((elo_total_correct / total_games * 100)|round(1) if total_games > 0 else 0) }}%</div>
                    </div>
                    <div style="background: rgba(255,255,255,0.1); padding: 12px; border-radius: 8px;">
                        <div style="opacity: 0.9; margin-bottom: 5px;">XGBoost</div>
                        <div style="font-size: 1.4em; font-weight: bold;">{{ xgb_total_correct }}-{{ xgb_total_wrong }}</div>
                        <div style="opacity: 0.8; font-size: 0.9em; margin-top: 3px;">{{ ((xgb_total_correct / total_games * 100)|round(1) if total_games > 0 else 0) }}%</div>
                    </div>
                    <div style="background: rgba(255,255,255,0.1); padding: 12px; border-radius: 8px;">
                        <div style="opacity: 0.9; margin-bottom: 5px;">CatBoost</div>
                        <div style="font-size: 1.4em; font-weight: bold;">{{ cat_total_correct }}-{{ cat_total_wrong }}</div>
                        <div style="opacity: 0.8; font-size: 0.9em; margin-top: 3px;">{{ ((cat_total_correct / total_games * 100)|round(1) if total_games > 0 else 0) }}%</div>
                    </div>
                    <div style="background: rgba(255,255,255,0.15); padding: 12px; border-radius: 8px; border: 2px solid rgba(251,191,36,0.5);">
                        <div style="opacity: 0.9; margin-bottom: 5px;">🏆 Meta</div>
                        <div style="font-size: 1.4em; font-weight: bold;">{{ meta_correct }}-{{ meta_wrong }}</div>
                        <div style="opacity: 0.8; font-size: 0.9em; margin-top: 3px;">{{ meta_accuracy }}%</div>
                    </div>
                </div>
            </div>
        </div>
...
NFL_WEEKLY_RESULTS_TEMPLATE = BASE_TEMPLATE.replace(
    ...
    {% if weekly_results %}
        ...
        <div style="background: linear-gradient(135deg, #10b981, #059669); border-radius: 15px; padding: 25px; margin-bottom: 30px; text-align: center;">
            ...
            {% set elo_ns = namespace(correct=0, total=0) %}
            {% set xgb_ns = namespace(correct=0, total=0) %}
            {% set cat_ns = namespace(correct=0, total=0) %}
            {% for week_num in weekly_results|dictsort %}
                {% set week_data = weekly_results[week_num[0]] %}
                {% set elo_ns.correct = elo_ns.correct + week_data.elo.correct %}
                {% set elo_ns.total = elo_ns.total + week_data.elo.total %}
                {% set xgb_ns.correct = xgb_ns.correct + week_data.xgboost.correct %}
                {% set xgb_ns.total = xgb_ns.total + week_data.xgboost.total %}
                {% set cat_ns.correct = cat_ns.correct + week_data.catboost.correct %}
                {% set cat_ns.total = cat_ns.total + week_data.catboost.total %}
            {% endfor %}
            ...
            <div style="margin-top: 20px; padding-top: 20px; border-top: 2px solid rgba(255,255,255,0.3);">
                ...
                <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px; font-size: 0.95em;">
                    <div style="background: rgba(255,255,255,0.1); padding: 12px; border-radius: 8px;">
                        <div style="opacity: 0.9; margin-bottom: 5px;">Elo</div>
                        <div style="font-size: 1.4em; font-weight: bold;">{{ elo_ns.correct }}-{{ elo_ns.total - elo_ns.correct }}</div>
                        <div style="opacity: 0.8; font-size: 0.9em; margin-top: 3px;">{{ ((elo_ns.correct / elo_ns.total * 100)|round(1) if elo_ns.total > 0 else 0) }}%</div>
                    </div>
                    <div style="background: rgba(255,255,255,0.1); padding: 12px; border-radius: 8px;">
                        <div style="opacity: 0.9; margin-bottom: 5px;">XGBoost</div>
                        <div style="font-size: 1.4em; font-weight: bold;">{{ xgb_ns.correct }}-{{ xgb_ns.total - xgb_ns.correct }}</div>
                        <div style="opacity: 0.8; font-size: 0.9em; margin-top: 3px;">{{ ((xgb_ns.correct / xgb_ns.total * 100)|round(1) if xgb_ns.total > 0 else 0) }}%</div>
                    </div>
                    <div style="background: rgba(255,255,255,0.1); padding: 12px; border-radius: 8px;">
                        <div style="opacity: 0.9; margin-bottom: 5px;">CatBoost</div>
                        <div style="font-size: 1.4em; font-weight: bold;">{{ cat_ns.correct }}-{{ cat_ns.total - cat_ns.correct }}</div>
                        <div style="opacity: 0.8; font-size: 0.9em; margin-top: 3px;">{{ ((cat_ns.correct / cat_ns.total * 100)|round(1) if cat_ns.total > 0 else 0) }}%</div>
                    </div>
                    <div style="background: rgba(255,255,255,0.15); padding: 12px; border-radius: 8px; border: 2px solid rgba(251,191,36,0.5);">
                        <div style="opacity: 0.9; margin-bottom: 5px;">🏆 Meta</div>
                        <div style="font-size: 1.4em; font-weight: bold;">{{ ns.total_correct }}-{{ meta_wrong }}</div>
                        <div style="opacity: 0.8; font-size: 0.9em; margin-top: 3px;">{{ meta_accuracy }}%</div>
                    </div>
                </div>
            </div>
...
@app.route('/')
def landing_page():
    """Landing page with sport selector (NO unified dashboard). Order is controlled by SPORTS_ORDER."""
    SPORTS_ORDER = ['NBA', 'NHL', 'NFL', 'NCAAB', 'NCAAF', 'MLB', 'WNBA']
    live_sports = {'NBA', 'NHL', 'NFL', 'NCAAB', 'NCAAF'}

    # Precompute accuracies (if you want to display later)
    accuracies = {s: get_landing_accuracy(s) for s in SPORTS_ORDER}
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .sports-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 25px; margin-bottom: 30px; }
        .sport-card { background: white; border-radius: 16px; padding: 35px; text-align: center; cursor: pointer; transition: all 0.3s ease; box-shadow: 0 4px 15px rgba(0,0,0,0.2); text-decoration: none; color: inherit; }
        .sport-card:hover { transform: translateY(-8px); box-shadow: 0 8px 25px rgba(0,0,0,0.3); }
        .sport-icon { font-size: 4em; margin-bottom: 15px; }
        .sport-name { font-size: 1.8em; font-weight: 700; margin-bottom: 8px; color: #333; }
        .sport-status { font-size: 1em; color: #666; margin-bottom: 12px; }
        .active { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; }
...
<body>
    <div class=\"container\">
        ...
        <div class=\"sports-grid\">
            ...
            <a href=\"/sport/{{ s }}/predictions\" class=\"sport-card {% if s in live_sports %}active{% endif %}\">
                <div class=\"sport-icon\">{{ SPORTS[s].icon }}</div>
                <div class=\"sport-name\">{{ SPORTS[s].name }}</div>
                <div class=\"sport-status\">{% if s in live_sports %}Live Now{% else %}Offseason{% endif %}</div>
            </a>
            {% endfor %}
        </div>
        <div class=\"footer\">
    ...
    """, SPORTS=SPORTS, SPORTS_ORDER=SPORTS_ORDER, live_sports=live_sports, accuracies=accuracies)
...
def sport_spread_total_results(sport):
    """Track accuracy of spread and total predictions against actual scores"""
    if sport not in SPORTS:
        return "Sport not found", 404
    ...
    # Get completed games with scores from database
    conn = get_db_connection()
    cursor = conn.cursor()
    
    completed_games = cursor.execute('''
    ...
    total_spread_margin = 0
    
    for game in completed_games:
        # Get predicted scores using ScorePredictor
        try:
            pred_home, pred_away, pred_spread, pred_total = predictor.predict_score(
            ...
            if pred_total:
                ...
                total_diff = actual_total - pred_total
            else:
                total_result = 'N/A'
                total_diff = None
            
            # Determine spread accuracy
            if pred_spread:
                spread_diff = abs(actual_spread - pred_spread)
                total_spread_margin += spread_diff
                ...
                spread_correct = (pred_winner == actual_winner)
            ...
            results.append({
                ...
                'predicted_total': round(pred_total, 1) if pred_total else 'N/A',
                'total_result': total_result,
                'total_diff': round(total_diff, 1) if total_diff is not None else 'N/A',
                'actual_spread': round(actual_spread, 1),
                'predicted_spread': round(pred_spread, 1) if pred_spread else 'N/A',
                'spread_diff': round(spread_diff, 1) if spread_diff is not None else 'N/A',
                'spread_correct': '✅' if spread_correct else '❌' if spread_correct is not None else 'N/A'
            })
        except Exception as e:
            logger.error(f"Error calculating results for {game['game_id']}: {e}")
            continue
    
    # Calculate overall stats
    total_games = len(results)
    over_pct = (total_over / total_games * 100) if total_games > 0 else 0
    under_pct = (total_under / total_games * 100) if total_games > 0 else 0
    spread_correct_pct = (total_spread_correct / total_games * 100) if total_games > 0 else 0
    avg_spread_margin = (total_spread_margin / total_games) if total_games > 0 else 0
...
<html>
<head>
    <title>{{ sport_info.name }} - Spread & Total Results</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body {
            font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;
        ...
        .stat-card h3 { color: #a78bfa; font-size: 0.9em; margin-bottom: 10px; }
        .stat-card .value { font-size: 2.5em; font-weight: 700; color: #fbbf24; }
        .results-table { width: 100%; border-collapse: collapse; background: rgba(255,255,255,0.05); border-radius: 12px; overflow: hidden; }
        .results-table th { background: rgba(139, 92, 246, 0.3); padding: 15px; text-align: left; font-weight: 600; }
        .results-table td { padding: 12px 15px; border-top: 1px solid rgba(255,255,255,0.1); }
        .results-table tr:hover { background: rgba(255,255,255,0.03); }
        .over { color: #10b981; font-weight: 600; }
        .under { color: #f87171; font-weight: 600; }
    </style>
</head>
<body>
    <div class="container">
        ...
        <div class="header">
            <h1>{{ sport_info.icon }} {{ sport_info.name }} - Spread & Total Results</h1>
            <p>Tracking accuracy of predicted spreads and totals vs actual game scores</p>
        </div>
        ...
        <div class="tabs">
            ...
            <a href="/sport/{{ sport }}/results" class="tab">🎯 Win/Loss Results</a>
            <a href="/sport/{{ sport }}/spreads" class="tab">📈 Spreads & Totals</a>
            <a href="/sport/{{ sport }}/spreads/results" class="tab active">📊 Spread/Total Results</a>
        </div>
        
        <div class="stats-grid">
            <div class="stat-card">
                <h3>TOTAL GAMES TRACKED</h3>
                <div class="value">{{ total_games }}</div>
            </div>
            <div class="stat-card">
                <h3>OVER RECORD</h3>
                <div class="value">{{ total_over }}-{{ total_under }} ({{ "%.1f"|format(over_pct) }}%)</div>
            </div>
            <div class="stat-card">
                <h3>SPREAD ACCURACY</h3>
                <div class="value">{{ "%.1f"|format(spread_correct_pct) }}%</div>
            </div>
            <div class="stat-card">
                <h3>AVG SPREAD MARGIN</h3>
                <div class="value">{{ "%.1f"|format(avg_spread_margin) }}</div>
            </div>
        </div>
        ...
        <table class="results-table">
            <thead>
                <tr>
                    ...
                    <th>Matchup</th>
                    <th>Actual Score</th>
                    <th>Total (Pred/Act)</th>
                    <th>O/U Result</th>
                    <th>Spread (Pred/Act)</th>
                    <th>Spread ✓</th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr>
                    ...
                    <td style="font-weight: 600;">{{ result.actual_score }}</td>
                    <td>{{ result.predicted_total }} / {{ result.actual_total }} ({{ result.total_diff }})</td>
                    <td class="{% if result.total_result == 'OVER' %}over{% elif result.total_result == 'UNDER' %}under{% endif %}">{{ result.total_result }}</td>
                    <td>{{ result.predicted_spread }} / {{ result.actual_spread }} ({{ result.spread_diff }})</td>
                    <td>{{ result.spread_correct }}</td>
                </tr>
                {% endfor %}
            </tbody>
        </table>
    ...
    return render_template_string(
        ...
        over_pct=over_pct,
        under_pct=under_pct,
        spread_correct_pct=spread_correct_pct,
        avg_spread_margin=avg_spread_margin
    )
